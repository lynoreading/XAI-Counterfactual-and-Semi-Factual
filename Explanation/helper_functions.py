...
def load_models(CNN, Generator):
	...
	G.load_state_dict  (torch.load('weights/generator.pth',   map_location='cpu')) # pretrained weight data
...
def load_models_Fashion(CNN, Generator):
	G = Generator(ngpu=1)
	cnn = CNN()
 
	G.load_state_dict  (torch.load('generator_weights.pth', map_location='cpu')) # pretrained weight data
	cnn.load_state_dict(torch.load('cnn_weights.pth', map_location='cpu'))
...
def acquire_feature_probabilities(target_class, cnn, original_query_img=None, alpha=0.05):
	query_features = cnn(original_query_img)[1][0]			# the last second layer nerual features
	digit_weights = cnn.classifier[0].weight[target_class]  # the true label weight
...
def acquire_feature_probabilities_Fashion(target_class, cnn, original_query_img=None, alpha=0.05):
	query_features = cnn(original_query_img)[1][0]			# the last second layer nerual features
	digit_weights = cnn.classifier[0].weight[target_class]  # the true label weight
	...
	for i in range(len(query_features)):
		...
		if np.all(data == 0):
			...
			replacement_value_1 = 1e-4
			replacement_value_2 = 1e-5
			num_elements = data.size
			num_half = num_elements // 2

			data[:num_half] += replacement_value_1
			data[num_half:] += replacement_value_2
...
def output_label(label, reverse=False):
	output_mapping = {
		0: "T-shirt/TOP",
		1: "Trouser",
		2: "Pullover",
	...
	if not reverse:
		return output_mapping.get(label)
	else:
		# Create a reverse mapping
		reverse_mapping = {v: k for k, v in output_mapping.items()}
