...
def load_models(CNN, Generator):
	G = Generator(ngpu=1)
	cnn = CNN()
	G.load_state_dict  (torch.load('weights/generator_f.pth',   map_location='cpu'))
	cnn.load_state_dict(torch.load('weights/cnn_f.pth', map_location='cpu'))
	G.eval()
	cnn.eval()
	return G, cnn
...
def acquire_feature_probabilities(target_class, cnn, original_query_img=None, alpha=0.05):
	...
	with open('data/collected_fashion.pickle', 'rb') as handle:
	#with open('data/pred_features.pickle', 'rb') as handle:
		dist_data = pickle.load(handle)
    
	#print(np.array(dist_data[0]).shape)
	fail_results = list()
...
def save_query_and_gan_xp_for_final_data(I_e, cnn, z, G, z_e, original_query_image, name, rand_num, target_class, original_query_pred):
	...
	plt.suptitle('original pred: ' + str(original_query_pred) + ', target class: ' + str(target_class)+ ',Alg:'+ str(name),y=0.05)
	plt.savefig('Explanations/a1/' + name + "_" + str(rand_num) + '.pdf')
	plt.close()
